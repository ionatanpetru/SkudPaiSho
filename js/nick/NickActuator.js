...
Nick.Actuator = function(gameContainer, isMobile, enableAnimations) {
	...
	var containers = setupPaiShoBoard(
		this.gameContainer, 
		Nick.Controller.getHostTilesContainerDivs(),
		Nick.Controller.getGuestTilesContainerDivs(), 
		true,
		Nick.Options.viewAsGuest ? ADEVAR_GUEST_ROTATE : ADEVAR_ROTATE
	);
...
Nick.Actuator.hostTeamTilesDivId = "hostTilesContainer";
Nick.Actuator.guestTeamTilesDivId = "guestTilesContainer";

Nick.Actuator.prototype.setAnimationOn = function(isOn) {
	this.animationOn = isOn;
};

Nick.Actuator.prototype.actuate = function(board, tileManager, markingManager, moveToAnimate, moveDetails) {
	var self = this;
...
Nick.Actuator.prototype.htmlify = function(board, tileManager, markingManager, moveToAnimate, moveDetails) {
	...
	self.clearContainerWithId(Nick.Actuator.hostTeamTilesDivId);
	self.clearContainerWithId(Nick.Actuator.guestTeamTilesDivId);
...
Nick.Actuator.prototype.addTile = function(tile, tileContainer, isCaptured) {
	if (!tile) {
		return;
	}
...
Nick.Actuator.prototype.clearContainer = function (container) {
	while (container.firstChild) {
		container.removeChild(container.firstChild);
	}
};

Nick.Actuator.prototype.clearContainerWithId = function(containerIdName) {
...
// Nick.Actuator.prototype.clearTileContainer = function(tile) {
...
Nick.Actuator.prototype.addLineBreakInTilePile = function(player) {
	var containerDivId = player === HOST 
								? Nick.Actuator.hostTeamTilesDivId
								: Nick.Actuator.guestTeamTilesDivId;
...
Nick.Actuator.prototype.addTeamTile = function(tile, player, isForTeamSelection) {
	var self = this;
...
Nick.Actuator.prototype.addBoardPoint = function(boardPoint, board, moveToAnimate, moveDetails) {
	var self = this;
	...
	if (!boardPoint.isType(NON_PLAYABLE)) {
		...
		if (boardPoint.isType(POSSIBLE_MOVE)) {
			...
			if (board.currentlyDeployingTileInfo && board.currentlyDeployingTileInfo.attributes
					&& board.currentlyDeployingTileInfo.attributes.includes(Nick.AttributeType.gigantic)) {
				// Gigantic!
...
Nick.Actuator.prototype.getCapturedTileFromMove = function(moveDetails) {
...
/* Can remove? */
Nick.Actuator.prototype.adjustBoardPointForGiganticDeploy = function(theDiv, boardPoint) {
...
Nick.Actuator.prototype.doAnimateBoardPoint = function(boardPoint, moveToAnimate, theImg, theDiv, moveDetails) {
...
Nick.Actuator.prototype.getTileSrcPath = function(tile) {
	if (Nick.Controller.isUsingCustomTileDesigns()) {
		return Nick.Controller.getCustomTileDesignsUrl();
	} else {
		var srcValue = "images/";
		var gameImgDir = "Nick/" + localStorage.getItem(Nick.Options.tileDesignTypeKey);
		srcValue = srcValue + gameImgDir + "/";
		return srcValue;
	}
};

Nick.Actuator.prototype.printBoard = function(board) {
